...
for name, countries in groups.items():
    ...
    # --- NEW: LAG ANALYSIS BLOCK ---
    lags = range(-7, 8)
    lag_corrs = []
    for l in lags:
        # We shift stringency to see if it predicts sentiment
    ...
    plt.title(f"Policy vs. Mood: {name.replace('_', ' ')}\n(Optimized r = {best_r:.3f} at {best_lag}d lag)")
    plt.savefig(f"{name}_timeseries.png", dpi=300)
    plt.close()
    ...
    plt.savefig(f"{name}_heatmap.png", dpi=300)
    plt.close()

    # 3. SAVE THE FINAL CSV (Using "With India" version)
    if "With_India" in name:
        df_merged.to_csv('enriched_research_data.csv', index=False)
        with open('lag_results.txt', 'w') as f:
